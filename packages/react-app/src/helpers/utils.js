...
//unlock
  ...
  const encryptBuffer = async (data) => {
    ...
    const encryptionKey = await crypto.subtle.generateKey({ 'name': 'AES-CBC', 'length': 256 }, false, ['encrypt', 'decrypt']);
...
function bytesToHex(bytes) {
  ...
  const fetchMembersOfClub = async (memberAddress) => {
    return await graphQLFetcher({
      query: `
        locks(where:{id: "${clubAddress}" }) {
          id
          members {
            id
            owner
      ...
      return data.data.locks[0].members;
    });
  ...
  const fetchAllClubsOfMember = async (memberAddress) => {
    return await graphQLFetcher({
      query: `
        keys (first: 25, where:{owner: "${memberAddress}"}) {
          id
          lock {
            id
            name
      ...
      const result = {};
      for(let k of data.data.keys) {
        result[k.lock.id] = result[k.lock.name];
      }
      return Object.keys(result).map(lid => ({id: lid, name: result[lid] });
    });
